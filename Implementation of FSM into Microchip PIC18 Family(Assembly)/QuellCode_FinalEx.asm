...
PortInit             ; Initialize PortA as Input and PORTB as Output
                    ; Summary
                    ; Clear PORTA, PORTB, select and load 0xff into TRISA, clear TRISB
                    ; select and load 0x07 into CMCON, then select and clear VRCON
                    ; select Option Register , load 11000000 into it to enable PORTB Pull-up, set interrupt to rising edge of RB0/INT pin
                    ; Select back PORTA, end
    ...
    clrf    TRISB
    banksel CMCON  ;switch to page with comparator contrl register
    movlw   0x07    ; 7 = B'111', CMCON<2:0> = 111 --> Comparator OFF
    movwf   CMCON
    banksel VRCON   ; Voltage reference control register
...
; Summary : Select Option register, and load 11000000 into it, to enable PORTB pull-ups and set interrupt as rising edge of RB0/INT Pin
; Inclusive OR WREG and f, then save data in f(OPTION_REG), OPTION_REG is 11000111
; where enable PORTB pull-ups and set interrupt as rising edge of RB0/INT Pin + Set prescaler rate for TMR0 ---> 1:256
; Load 10100000 into INTCON
; Enables all-unmasked(non-maskable interrupt is hardware interrupt that cannot be ignore) interrupts globally
; Disbales TMR0 interrupts.
; Select TMR0, and reload TMR0 with loading T0reload(=0) into TMR0 
...
TicTacInit  ; Initialize TicTac by loading TicLoad(d'255') into TicCnt
    movlw   Ticload 
...
; Note: InFlags and InPort are two different file registers
; InPorts get data from PORTA directly. 
; InFlags is used to flag up when reading InPort if Bell Button is fired.
    bcf     InFlags,BbF      
...
; If the current state is same as the previous state, will execute FSMdo1(jump to there), in FSMdo1, it checks whether the state downcounts finished or not,
;
...
; FSM       :   Check for State Transition,if state changed, shows that the machine now entred into that state,
            ;   hence, load Zeit to FSMTime, and load OutPattern to OutFlags. Executing that state's output.
            ;   If no state change, go FSMdo1. 
            ; Here, Current State is load into Temp : FSMStateP --> WREG --> Temp
            ; So that it always save the current state for offseting table's column index purposes.
            ; Else, when load next state's number, can't invoke stuff that relates with Current State as it is override.

;   FSMdo1    :   No State Transition, test timeout of that State(FSMTime), if Z = 1, means delay timeout, downcount finished, 
;               then call TimeExit to next State, save @FSMStateP
;               Here, is when there's no chnage in state, but when Z=0, means still counting, will jump to FSMdo2 to downcount.
;               in btfss STATUS,Z, it is actually test FSMTime is zero of not(by mov FSMTime,f). If Zero, means Z = 1,
;               Will call TimeExit, which is the next state after delay timeout and load into FSMStateP.
;               Before that, it loads the current state(from Temp, see FSM block) to the WREG, to offset in the table in TimeExit.

;   FSMdo2    : Test for Downcount lifetime & Down counting of FSMTime takes place here.

;   FSMdo3      : Test Bell Button & go to next state(depends on stimulus). If return literal from the EventMask table is 0, and after AND operation the Z of STATUS
;              will be 1. Z = 1, shows that the logical operation results in zero. Means that this state only listen to timer interrupt instead of
;              any stimulus(in this project would be the Bell Button)
;               When the logical operation does not result in Zero, when Z = 1, means that this state listen stimulus. Which then will take data from
;               Temp to the WREG for offsetting to get the right Next State index from table of EventExit.
;
;   FSMexit     : Check Previous State & Next State same or not, if there's change of State, where Temp(current state) XOR with FSMStateP(Next State),
;                 then yields 1, cause different state(read up XOR truth table), Z in STATUS = 0, as not Zero. Then will clear FSMF, or FSMF = 0,
;                 to indicates there's change of state, then exit FSM and produce output and loop again.
;                 ...Else when no change of state, will not clear FSMF, ot FSMF = 1(no state change), and straight produce outputs
;
...
FSM         ;Tabellen gesteuerter Zustandsautomat/Finite State Machine
    ...
    call    Zeit            ; So when FSMF=0, will execute this line, and call time. This means the machine has entered new state and start to count!!!!!!
...
FSMdo1                  ; No State Transition, test timeout, if Z = 1, means delay timeout, downcount finished, then call TimeExit to next State, save @FSMStateP
                        ...
                        ; How we got here? No change of state, from FSM code block where FSMF =1, goto FSMdo1(here), will executes these.
...
FSMdo3                  ; Test Bell Button & go to next state(depends on stimulus)
    movf    Temp,w      ; Temp is the Current State Index(S0,...,S7...). Move form Temp to WREG, for offsetting in the table purposes.
    call    EventMask   ; Call EventMask table to check which state and check if that state listen to Bb or not.
                        ...
                        ; EventMask table returns 0 if the state does not depends on Stimulus, but only Timer Interrupt instead.
    andwf   InFlags,w   ; InFlags: Bit 0 = BbF, Bit 1 = nBbF
                        ; InFlags AND WREG and store in WREG.
                        ; Supposedly InFlags and EventMask Literal reutrn(now in WREG) should be the same. After AND, save in the WREG.
                        ...
                        ; So if the stat don't listen to Stimulus, will return literal 0 form EventMask table, Z = 1, will execute "FSMexit", where in "FSMexit"
...
FSMexit                 ; Check Previous State & Next State same or not, then exit FSM and produce output and loop again.
    ...
    movf    Temp,w      ; Temp is previous/current state
                        ...
                        ;                Z = 0, NS != PS, there's change of state, clear FSMF, then produce output.
                        ;
...
OutDrv
    movf    OutFlags,w  ; OutFlags gets value from OutPattern table 
...
; Zeit : Time delay in second. So if no delay(only listen to stimulus), it will be Zero. But the TimeExit = the same State
; Outpattern: Output pattern. The string contains bits, where each and every bits(if 8 bits are used) have its own usage.
; TimeExit: Next State after timeout
; EventMask: Check whether the current state listen to Stimulus or not. If only depends on delay to timeout, then it will be equal to Zero
; EventExit: Next State after Stimulus is activated.
;
;
;-----------------------------------Tabellen;-----------------------------------
;
